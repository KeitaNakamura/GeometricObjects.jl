@testset "Polyline" begin
    ...
    C = coordinates(poly)
    @test distance(poly, Vec(0.0, -1.0), 2.0, include_tips = true)  == C[1] - Vec(0.0, -1.0)
    ...
    @test distance(poly, Vec(-1.0, 20.0), 2.0, include_tips = true)  == C[3] - Vec(-1.0, 20.0)
    @test distance(poly, Vec(-1.0, 20.0), 2.0, include_tips = false) == nothing
    @test distance(poly, Vec(0.0, 21.0), 2.0, include_tips = true)  == C[3] - Vec(0.0, 21.0)
